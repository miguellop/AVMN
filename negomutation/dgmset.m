...
%                       [ 'continuous' | 'discrete' ]
%   DomainInterval      - Domain interval
%                       [ vector with two scalars ]
...
if (nargin == 0) && (nargout == 0)
    ...
    fprintf('               AgentType: [ ''quotas'' | ''rank'' | ''truevoting'' | {''quotas''} ]\n');
    ...
    fprintf('              DomainType: [ ''discrete'' | ''continuous'' ]\n');
    fprintf('          DomainInterval: [ vector with two scalars ]\n\n');
...
switch field
    case {'AgentType','MediationType','DomainType'}
        if ~isa(value,'char') 
