function state = negomutation(options)
    ...
    for i=1:options.Nsets
        ...
        for j=1:options.Nexp
            fprintf(' %i - ', j);
            state.alfa = 0.00001; %valor alfa del quantificador (lineal)
            ...
            thisPopulation = creationfcn(options.PopulationSize,options.Ni,options.DomainType,options.DomainInterval);
            ...
            for k=1:options.Generations
                ...
                %***meter weights y owa para calcular la agregación
                %(parámetro en aggreagtionfcn(score,w);
                expectation = aggregationfcn(score,getowaweights(getquantifier(1.1,state.alfa),options.Nag));
                ...
                thisPopulation = mutationfcn(thisPopulation(selection,:),state.PopulationSize,'A',options.DomainInterval);
